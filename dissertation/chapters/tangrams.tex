...
\chapter{Characterizing conventions: the dynamics of structure and content in repeated reference}
...
Certain basic descriptive questions remain unanswered, and important theoretical constructs remain poorly operationalized. 
...
For example, while reduction in utterance length across a repeated reference game has been widely replicated, a precise characterization \emph{what} gets reduced, and \emph{how}, has remained elusive.
...
Similarly, while theoretical definitions of constructs like arbitrariness or stability have loomed over the theoretical analysis of conventions \cite{Lewis69_Convention}, it has been unclear how exactly to measure the extent to which these properties hold in a repeated reference game and how they may change over the course of interaction. 
Without addressing these gaps, it is difficult to distinguish among different models.
...
In this chapter, we present a large corpus of messages sent in a web-based reference game replicating the classic Tangrams task \cite{XXX}.
Analyzing such rich natural language data was limited at the time of classic work, but
has become newly tractable using modern techniques from natural language processing (NLP). 
...
\section{Results: How do meanings change over time?}

Our inferential account makes three key predictions about how speakers change the content of their referring expressions over time.
...
An influential result from is that conceptual pacts for referring are established \emph{collaboratively} \cite[see also \cite{KraussWeinheimer66_Tangrams, GarrodFayLeeOberlanderMacLeod07_GraphicalSymbolSystems}]{clarkReferringCollaborativeProcess1986}: 
