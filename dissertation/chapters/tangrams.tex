...
For each tangram, we computed the pairwise cosine similarity $$\cos \theta_{ij} = \frac{u_i \cdot u_j}{\| u_i\| \| u_j \|}$$ between embeddings of all utterances \emph{within} a game and also \emph{across} games. 
The distributions of these values are shown in Fig. \ref{}. 
We bootstrapped the mean of each of these distributions, resampling different pairs and different tangrams with replacement, to obtain a confidence interval. 
We found that this ratio was significantly lower than unity, $CI: [], p = XXX$, consistent with the idea that different pairs adopt different conventions while a single pair tends to keep using a convention once established.

Having established that there is variance across pairs, how do individual games 
In Fig. \ref{fig:tsne}, we visualize the trajectory of an example tangram for each game in a common vector space using dimensionality reduction techniques.
First, we take the first 50 components recovered by Principal Components Analysis (PCA). 
We then run the t-SNE algorithm on these lower-dimensional representations, which stochastically embeds the semantic representations of each utterance used to refer to a given tangram in a common 2D vector space. 
...
To test these observations more rigorously, we conducted three analyses directly on the semantic vectors (Fig. \ref{fig:similarity}).
...
Another way to quantify convergence and divergence across different pairs is to examine the discrete \emph{distribution of words} that each pair uses to refer to each tangram.
...
A third key prediction concerns \emph{which} content is dropped and which is preserved? 
