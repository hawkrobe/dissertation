...
\caption{(A) similar reduction in \# words per tangram for both variants of the task (B) word counts broken down by part of speech, combined across both variants.}
\label{fig:feedback}
\end{figure}
...
To test this prediction, we coded whether the listener sent a message or not on each trial. 
In a mixed-effects logistic regression with a fixed effect of repetition and random intercepts and slopes for each target and pair of participants, we found that the probability of the listener sending a message decreased significantly over the game $(b=-0.85, t = -9.1, p < 0.001; Fig. \ref{fig:reduction}A)$.
...
\todo[inline]{Put paragraph about effect of feedback Fig. }
%, and there is a small but significant effect of the (logged) number of initial listener messages on overall \% reduction in the number of words used by the director, $r = 0.26, 95\% CI = [0.57, 0.45], p = 0.014$.
...
This decrease replicates a highly reliable reduction effect found throughout the literature on iterated reference games (Brennan \& Clark, 1996; Krauss \& Weinheimer, 1964), although due to our text-based (vs.~spoken) interface, participants in our task used many fewer words overall than previously reported. 
...
In this section, we delved into the dynamics of semantic content across repeated reference.
