...
\begin{figure}[t]
\centering
\includegraphics[scale=.7]{frontfig.pdf}
...
In this paper, we introduce a generic approach for transforming neural language models into \emph{adaptive} models that can be deployed in real-time interactions with other agents (Fig.~\ref{fig:refgame}). 
...
\begin{algorithmic}
    \STATE {\textbf{Input}: $\Theta_t$: weights at time $t$}
    \STATE {\textbf{Output}: $\Theta_{t+1}$: updated weights}
...
\begin{figure*}[t]
\centering
\includegraphics[scale=0.67]{combinedResults.pdf}
\vspace{-1em}
